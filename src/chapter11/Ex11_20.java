...
public class Ex11_20 {
	...
	static void inputRecord() {
		...
		while (true) {
			System.out.print(">>");
			/*
			 * (2) 아래의 로직에 맞게 코드를 작성하시오. 1. Scanner를 이용해서 화면으로 부터 데이터를 입력받는다.(','를 구분자로)
			 * 2.입력받은 값이 q 또는 Q이면 메서드를 종료하고, 그렇지 않으면 입력받은 값으로 Student인스턴스를 생성하고 record에
			 * 추가한다. 3. 입력받은 데이터에서 예외가 발생하면, "입력오류입니다."를 보여주고 다시 입력받는다. 4. q 또는 Q가 입력될 때까지
			 * 2~3의 작업을 반복한다.
			 */
			String[] st = s.nextLine().split(",");
			if (st[0].equalsIgnoreCase("q")) {
				break;
			} else {
				try {
					String name = st[0];
					int ban = Integer.parseInt(st[1]);
					int no = Integer.parseInt(st[2]);
					int kor = Integer.parseInt(st[3]);
					int eng = Integer.parseInt(st[4]);
					int math = Integer.parseInt(st[5]);
